// 企业资讯详情模块 - 处理企业资讯详情页面相关逻辑

// 复用公司页面的资讯详情API调用函数
async function fetchCompanyNewsDetail(newsId) {
    console.log('获取企业资讯详情，ID:', newsId);
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (isSuccess) {
                ...
                console.log('企业资讯详情数据:', resultData);
                return resultData;
            } else {
                console.warn('企业资讯详情API返回错误:', data.message || data.msg || '未知错误');
                return null;
            }
        } else {
            console.error('企业资讯详情API请求失败:', response.status);
            return null;
        }
    } catch (error) {
        console.error('获取企业资讯详情失败:', error);
        return null;
    }
}

class CompanyNewsDetailManager {
    constructor() {
    ...
    // 初始化企业资讯详情页面
    init() {
        console.log('初始化企业资讯详情页面');
        
        // 优先尝试从 sessionStorage 获取新闻ID
        const newsData = sessionStorage.getItem('currentCompanyNewsDetail');
        if (newsData) {
            try {
                ...
                console.error('解析企业新闻数据失败:', error);
                this.showErrorMessage('企业资讯数据加载失败');
            }
        } else {
            console.warn('没有找到企业资讯数据');
            this.showErrorMessage('未找到企业资讯数据，请返回企业详情页重新选择');
        }
    }
    ...
    async loadNewsDetailFromAPI(newsId) {
        ...
        try {
            const newsDetail = await fetchCompanyNewsDetail(newsId);
            if (newsDetail) {
                // 从sessionStorage获取原始新闻数据以保留其他字段
                const originalNewsData = sessionStorage.getItem('currentCompanyNewsDetail');
                let originalNews = {};
                if (originalNewsData) {
                    try {
                        originalNews = JSON.parse(originalNewsData);
                    } catch (e) {
                        console.warn('解析原始企业新闻数据失败:', e);
                    }
                }
                ...
                if (newsData) {
                    try {
                        ...
                        this.showErrorMessage('企业资讯详情加载失败，请稍后重试');
                    }
                } else {
                    this.showErrorMessage('企业资讯详情加载失败，请稍后重试');
                }
            ...
            console.error('加载企业资讯详情失败:', error);
            // 回退到缓存数据
            const newsData = sessionStorage.getItem('currentCompanyNewsDetail');
            if (newsData) {
                try {
    ...
    // 显示加载状态
    showLoadingState() {
        const content = document.getElementById('company-news-detail-content');
        if (content) {
            content.innerHTML = `
                <div class="text-center py-12">
                    <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
                    <p class="text-lg text-gray-700">正在加载企业资讯详情...</p>
                </div>
            `;
        }
    }
        ...
        if (content) {
            content.innerHTML = `
                <div class="text-center py-12">
                    ...
                    <button onclick="switchView('company')" class="mt-4 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition">
                        返回企业详情
                    </button>
                </div>
            `;
...
// 全局实例
const companyNewsDetailManager = new CompanyNewsDetailManager();

// 页面加载完成后初始化
document.addEventListener('DOMContentLoaded', function() {
    document.addEventListener('viewChanged', function(e) {
        if (e.detail.viewName === 'company-news-detail') {
            console.log('初始化企业资讯详情视图');
            companyNewsDetailManager.init();
        }
    });
});

// 导出函数供全局使用
window.loadCompanyNewsDetail = () => companyNewsDetailManager.init();